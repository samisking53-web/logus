...
import { readDraft, saveAgreementsToDraft } from "@/lib/local-auth";
import { readNextParam, withNext } from "@/lib/next-param";
import { AGREEMENTS, type AgreementKey } from "@/lib/terms";
...
export function TermsForm() {
  ...
  // (Next.js는 떠난 화면을 숨겨서 보관하므로, 뒤로 돌아왔을 때 버튼이 멈춰 있지 않게 한다)
  ...
  // 새로고침했을 때 이미 체크해 둔 내용이 있으면 되살린다
  useEffect(() => {
    const saved = readDraft().agreements;
    if (saved) {
      setChecked(Object.fromEntries(saved.map((a) => [a.key, a.agreed])) as Record<AgreementKey, boolean>);
    }
  }, []);
  ...
  function submit() {
    setError(null);
    try {
      // 체크한 항목과 안 한 항목(선택 항목)을 약관 버전·동의 시각과 함께 적어 둔다.
      // 계정은 3단계에서 만들어지고, 그때 이 기록도 계정에 함께 저장된다
      saveAgreementsToDraft(checked);
    } catch (err) {
      setError(err instanceof Error ? err.message : "저장하지 못했어요. 다시 시도해 주세요.");
      return;
    }
    startMoving(() => router.push(withNext("/signup/account", readNextParam())));
  }

  return (
    <>
      <StepHeader step={1} onBack={() => router.push(withNext("/login", readNextParam()))} />
      ...
      <NextButton className="mt-3" disabled={!requiredChecked || moving} onClick={submit}>
        {!requiredChecked ? "필수 항목에 동의해 주세요" : "다음"}
      </NextButton>
    </>
  );
}
